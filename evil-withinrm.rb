...
$full_logging_path = "#{Dir.home}/evil-withinrm-logs"
...
class EvilWinRM
  ...
  def arguments
    ...
    optparse = OptionParser.new do |opts|
      opts.banner = 'Usage: evil-withinrm -i IP -u USER [-s SCRIPTS_PATH] [-e EXES_PATH] [-P PORT] [-a USERAGENT] [-p PASS] [-H HASH] [-U URL] [-S] [-c PUBLIC_KEY_PATH ] [-k PRIVATE_KEY_PATH ] [-r REALM] [--spn SPN_PREFIX] [-l]'
  ...
  # Print script header
  def print_header
    print_message("evil-withinrm shell v#{VERSION}", TYPE_INFO, false)
  end
      ...
      $conn.shell(:powershell) do |shell|
        begin
          ...
          until command == 'exit' do
            ...
            if $colors_enabled
              @prompt = "#{colorize('*evil-withinrm*', 'red')}#{colorize(' PS ', 'yellow')}#{pwd}> "
              print @prompt
              command = gets.chomp
            else
              command = Readline.readline("*evil-withinrm* PS #{pwd}> ", true).encode!('UTF-8', 'binary', invalid: :replace, undef: :replace, replace: '')
            end
            $logger&.info("*evil-withinrm* PS #{pwd} > #{command}")
